  ...
  T* new_value_cache)
{
  ...
  if (has_layer_state) {
    if (use_past && static_kv) {
      CHECK_ROCM(hipMemcpyAsync(new_key_cache, key_cache, kv_sequence_length * BHN * sizeof(T), hipMemcpyDeviceToDevice, stream));
      CHECK_ROCM(hipMemcpyAsync(new_value_cache, value_cache, kv_sequence_length * BHN * sizeof(T), hipMemcpyDeviceToDevice, stream));
    } else {
      CHECK_ROCM(hipMemcpyAsync(new_key_cache, k, kv_sequence_length * BHN * sizeof(T), hipMemcpyDeviceToDevice, stream));
      CHECK_ROCM(hipMemcpyAsync(new_value_cache, v, kv_sequence_length * BHN * sizeof(T), hipMemcpyDeviceToDevice, stream));
    }
  }
  ...
  if (use_past && static_kv) {
    if (!ROCBLAS_CALL(rocblasGemmStridedBatchedHelper(
      rocblas, rocblas_operation_transpose, rocblas_operation_none, kv_sequence_length, sequence_length, head_size, &alpha, key_cache, head_size, strideA,
      q, head_size, strideB, &zero, scratch1, kv_sequence_length, temp_matrix_size, BN))) {
      return false;
    }
  } else {
    if (!ROCBLAS_CALL(rocblasGemmStridedBatchedHelper(
      rocblas, rocblas_operation_transpose, rocblas_operation_none, kv_sequence_length, sequence_length, head_size, &alpha, k, head_size, strideA,
      q, head_size, strideB, &zero, scratch1, kv_sequence_length, temp_matrix_size, BN))) {
      return false;
    }
  }
  ...
  if (use_past && static_kv) {
    if (!ROCBLAS_CALL(rocblasGemmStridedBatchedHelper(
      rocblas, rocblas_operation_none, rocblas_operation_none, head_size, sequence_length, kv_sequence_length, &one, value_cache, head_size, strideA,
      scratch2, kv_sequence_length, temp_matrix_size, &zero, scratch3, head_size, strideB, BN))) {
      return false;
    }
  } else {
    if (!ROCBLAS_CALL(rocblasGemmStridedBatchedHelper(
      rocblas, rocblas_operation_none, rocblas_operation_none, head_size, sequence_length, kv_sequence_length, &one, v, head_size, strideA,
      scratch2, kv_sequence_length, temp_matrix_size, &zero, scratch3, head_size, strideB, BN))) {
      return false;
    }
  }
