...
bool LaunchLongformerSoftmaxKernel(
  ...
  CHECK_ROCM(hipMemcpyAsync(scratch2, temp_buffer, totalBytes, hipMemcpyHostToDevice, stream));
  ...
  for (int i = 0; i < batch_size; ++i) {
    if (global_count[i] > 0) {
      ...
      CHECK(rocblas_gemm_strided_batched_ex(rocblas,
                                            rocblas_operation_transpose,
                                            rocblas_operation_none,
                                            sequence_length,  // m
                                            global_count[i],  // n
...
bool LongformerQkvToContext(
    const hipDeviceProp_t& device_prop, rocblas_handle rocblas, hipStream_t stream,
