...
def get_model(args: argparse.Namespace):
    if args.benchmark_type in {"pt-eager", "pt-compile"}:
        ...
        ep = (
            ("MIGraphXExecutionProvider", {"device_id": args.device_id})
        )
...
def get_args():
    ...
    if "ort" in args.benchmark_type:
        ...
        elif args.execution_provider == "MIGraphXExecutionProvider":
            args.execution_provider = (args.execution_provider, {"device_id": args.device_id})
            args.device = "cuda"
...
def main():
    ...
    for batch_size, prompt_length in itertools.product(args.batch_sizes, args.prompt_lengths):
        ...
        try:
            ...
            unpadded_mask = inputs["attention_mask"]

            ################
            #pre-compile
            print("Running pre-compilation...")
            pc_unpadded_mask = unpadded_mask
            pc_outputs = outputs
            pc_inputs = inputs
            pc_all_token_ids = all_token_ids
            pc_current_length = pc_all_token_ids.shape[-1]
            atls, pc_outputs = run_inference(args, model, 1, pc_inputs, pc_outputs)
            if pc_outputs["logits"].shape[1] > 1:
                    prompt_end_indices = pc_inputs["attention_mask"].sum(1) - 1
                    idxs = (
                        prompt_end_indices.unsqueeze(dim=1)
                        .repeat(1, config.vocab_size)
                        .view(batch_size, 1, config.vocab_size)
                    )
                    next_token_logits = torch.gather(pc_outputs["logits"], 1, idxs).squeeze()
            else:
                next_token_logits = pc_outputs["logits"][:, -1, :]
            ...
            pc_all_token_ids = torch.cat([pc_all_token_ids, tokens_to_add], dim=-1)
            pc_current_length += 1

            # Update inputs for next inference run
            pc_inputs["input_ids"] = tokens_to_add
            # inputs["attention_mask"] = torch.cat(
            #     [inputs["attention_mask"], (~has_eos).to(torch.int64).reshape(batch_size, 1)], 1
            # )
            pc_unpadded_mask = torch.cat([pc_unpadded_mask, (~has_eos).to(torch.int64).reshape(batch_size, 1)], 1)
            pc_padded_mask = torch.nn.functional.pad(pc_unpadded_mask, (0, 4096 - pc_current_length), mode='constant', value=0)
            pc_inputs["attention_mask"] = pc_padded_mask

            # Set logits to zeros for next inference run and re-use memory buffer
            if pc_outputs["logits"].shape[1] != 1:
                pc_outputs["logits"] = pc_outputs["logits"][:, :1, :].contiguous()
            pc_outputs["logits"].zero_()
            # print()
            atls, pc_outputs = run_inference(args, model, 1, pc_inputs, pc_outputs)
            print("Finished pre-compilation.")
            #end pre-compile
            #################
            ...
            while current_length <= max_length:
                ...
                #     [inputs["attention_mask"], (~has_eos).to(torch.int64).reshape(batch_size, 1)], 1
                # )
                unpadded_mask = torch.cat([unpadded_mask, (~has_eos).to(torch.int64).reshape(batch_size, 1)], 1)
                padded_mask = torch.nn.functional.pad(unpadded_mask, (0, 4096 - current_length), mode='constant', value=0)
