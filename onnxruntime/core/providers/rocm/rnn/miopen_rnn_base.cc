...
Status MiopenRnnBase<T>::ReorganizeWeights(const Tensor* W, const Tensor* R, const Tensor* B,
  ...
  miopenHandle_t miopen_handle = ort_rocm_stream ? ort_rocm_stream->miopen_handle_ : DefaultMiopenHandle();
  ORT_RETURN_IF_ERROR(SetMiopenRnnWeightBias(miopen_handle, rnn_desc, fake_x_desc, target_w_desc,
                                            reorganized_w_data.get(), W_data, R_data, B_data, hip_stream));
...
Status MiopenRnnBase<T>::CacheMiopenRnnWeights(const OpKernelInfo& info) {
  ...
  if (get_W && get_R) {
    MiopenRNN tmp_rnn_desc;
    ORT_RETURN_IF_ERROR(tmp_rnn_desc.Set(DefaultMiopenHandle(),
                                         hidden_size_,
                                         RNN_NUM_LAYERS,
                                         miopen_dropout_desc_,
                                         miopen_direction_mode_,
                                         rnn_mode_,
...
Status MiopenRnnBase<T>::ComputeInternal(OpKernelContext* ctx) const {
  ...
  MiopenRNN rnn_desc;
  ORT_RETURN_IF_ERROR(rnn_desc.Set(GetMiopenHandle(ctx),
                                   hidden_size_,
                                   RNN_NUM_LAYERS,
                                   miopen_dropout_desc_,
                                   miopen_direction_mode_,
  ...
  auto workspace_rocm = GetScratchBuffer<void>(workspace_bytes, ctx->GetComputeStream());
  ...
  if (reverse_ || num_directions_ == 2) {
    ...
    if (Y != nullptr) {
      ...
      HIP_RETURN_IF_ERROR(hipMemcpyAsync(y_data, y_reorganized_data.get(), output_size * sizeof(T), hipMemcpyDeviceToDevice, Stream(ctx)));
    } else {
...
template <typename T>
void MiopenRnnBase<T>::SetZeroSequences(const int64_t zero_seq_index_cache_size,
