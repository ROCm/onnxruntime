...
Status ReduceRowSumOnMatrixA(hipStream_t stream, const int8_t* matrix, int32_t* row_sum, const int8_t offset, const MatMulComputeHelper& helper) {
  for (size_t batch = 0; batch < helper.OutputOffsets().size(); batch++) {
    hipLaunchKernelGGL(HIP_KERNEL_NAME(ReduceRowSumOnMatrixAKernel<static_cast<int>(GridDim::maxThreadsPerBlock)>), dim3(static_cast<int>(helper.M())), dim3(GridDim::maxThreadsPerBlock), 0, stream, matrix + helper.LeftOffsets()[batch],
...
Status ReduceColSumOnMatrixB(hipStream_t stream, const int8_t* matrix, int32_t* col_sum, const int8_t offset, const MatMulComputeHelper& helper) {
  for (size_t batch = 0; batch < helper.OutputOffsets().size(); batch++) {
    hipLaunchKernelGGL(HIP_KERNEL_NAME(ReduceColSumOnMatrixBKernel<static_cast<int>(GridDim::maxThreadsPerBlock)>), dim3(static_cast<int>(helper.N())), dim3(GridDim::maxThreadsPerBlock), 0, stream, matrix + helper.RightOffsets()[batch],
...
Status OffsetOutput(hipStream_t stream,
  ...
  if (a_offset && b_offset) {
    for (size_t batch = 0; batch < helper.OutputOffsets().size(); batch++) {
      hipLaunchKernelGGL(ComputeOffsetOfMatrixAB, dim3(static_cast<int>(helper.M())), dim3(GridDim::maxThreadsPerBlock), 0, stream, row_sum + batch * helper.M(),
          col_sum + batch * helper.N(),
    ...
    for (size_t batch = 0; batch < helper.OutputOffsets().size(); batch++) {
      hipLaunchKernelGGL(ComputeOffsetOfMatrixA, dim3(static_cast<int>(helper.M())), dim3(GridDim::maxThreadsPerBlock), 0, stream, col_sum + batch * helper.N(),
          output + helper.OutputOffsets()[batch],
          static_cast<int32_t>(helper.N()));
    }
  } else if (b_offset) {
    for (size_t batch = 0; batch < helper.OutputOffsets().size(); batch++) {
      hipLaunchKernelGGL(ComputeOffsetOfMatrixB, dim3(static_cast<int>(helper.M())), dim3(GridDim::maxThreadsPerBlock), 0, stream, row_sum + batch * helper.M(),
          output + helper.OutputOffsets()[batch],
          static_cast<int32_t>(helper.N()));
    }
  }
