...
struct MIGraphX_Provider final : Provider {
  ...
  void UpdateProviderOptions(void* provider_options, const ProviderOptions& options) override {
    ...
    if (auto str_size = internal_options.int8_calibration_table_name.length(); str_size == 0) {
      migx_options->migraphx_int8_calibration_table_name = nullptr;
    } else {
      auto dest{static_cast<char*>(std::malloc(str_size + 1))};
#ifdef _MSC_VER
    ...
    if (auto str_size = internal_options.model_cache_dir.string().length(); str_size == 0) {
      migx_options->migraphx_cache_dir = nullptr;
    } else {
      auto dest{static_cast<char*>(std::malloc(str_size + 1))};
#ifdef _MSC_VER
      strncpy_s(dest, str_size + 1, internal_options.model_cache_dir.string().c_str(), str_size);
#else
      strncpy(dest, internal_options.model_cache_dir.string().c_str(), str_size);
#endif
      dest[str_size] = '\0';
      migx_options->migraphx_cache_dir = static_cast<const char*>(dest);
    }
