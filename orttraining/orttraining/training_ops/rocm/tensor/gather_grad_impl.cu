...
// hipcub missing DiscardOutputIterator
#include "orttraining/training_ops/rocm/tensor/discard_output_iterator.h"
...
void GetSortedIndices(
  ...
  auto dY_indices = rocm_kernel.template GetScratchBuffer<TIndex>(num_gathered_indices);
  ...
  auto dX_indices_sorted = rocm_kernel.template GetScratchBuffer<TIndex>(num_gathered_indices);
  auto dY_indices_sorted = rocm_kernel.template GetScratchBuffer<TIndex>(num_gathered_indices);
  ...
  auto temp_storage = rocm_kernel.template GetScratchBuffer<void>(temp_storage_size_bytes);
  HIP_CALL_THROW(hipcub::DeviceRadixSort::SortPairs(
...
IAllocatorUniquePtr<T> GetOffsetsFromCounts(
  ...
  auto offsets = rocm_kernel.template GetScratchBuffer<T>(num_counts);
  size_t temp_storage_size_bytes = 0;
...
void PartialSumsImpl(
  ...
  auto per_segment_partial_segment_counts = rocm_kernel.template GetScratchBuffer<SegmentIndex_t>(num_segments);
  {
  ...
  auto per_segment_partial_segment_offsets = GetOffsetsFromCounts(
      rocm_kernel, per_segment_partial_segment_counts.get(), num_segments);
  ...
  auto partial_segment_offsets = rocm_kernel.template GetScratchBuffer<GatheredIndexIndex_t>(host_num_partial_segments);
  {
...

  {
    ...
    auto partial_segment_sums = rocm_kernel.template GetScratchBuffer<AccumulationType_t<T>>(
...
void Impl(
  ...
  auto segment_counts = rocm_kernel.template GetScratchBuffer<GatheredIndexIndex_t>(num_gathered_indices);
  {
    auto num_segments = rocm_kernel.template GetScratchBuffer<SegmentIndex_t>(1);
    size_t temp_storage_size_bytes = 0;
    HIP_CALL_THROW(hipcub::DeviceRunLengthEncode::Encode(
        nullptr, temp_storage_size_bytes,
        dX_indices_sorted.get(), DiscardOutputIterator<TIndex>{}, segment_counts.get(),
    ...
    // CPU/GPU sync!
    HIP_CALL_THROW(hipMemcpy(
        &host_num_segments, num_segments.get(), sizeof(SegmentIndex_t), hipMemcpyDeviceToHost));
  }

  // get largest segment size and use that to select implementation
  GatheredIndexIndex_t host_max_segment_count = 0;
  {
    auto max_segment_count = rocm_kernel.template GetScratchBuffer<GatheredIndexIndex_t>(1);
    ...
    // CPU/GPU sync!
    HIP_CALL_THROW(hipMemcpy(
        &host_max_segment_count, max_segment_count.get(), sizeof(GatheredIndexIndex_t), hipMemcpyDeviceToHost));
  }
  ...
  if (host_max_segment_count <= kMaxSegmentSizeThreshold) {
    ...
    auto segment_offsets = GetOffsetsFromCounts(
        rocm_kernel, segment_counts.get(), host_num_segments);
